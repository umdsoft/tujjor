...
exports.sendCode = async (req, res) => {
  ...
  if (!client) {
      ...
      .then(() => {
        SMS(phone, code)
          .then(() => {
            res
              .status(200)
              .json({success: true, message: "confirmation code sent!"});
          })
          .catch((e) => {
            throw e;
          });
    ...
    Client.findByIdAndUpdate({_id: client._id}, {$set: {code: code}})
      .then(() => {
        SMS(phone, code)
          .then(() => {
            res
...
exports.resetPassword = async (req, res) => {
  ...
  if (!(req.body.password && req.body.password.length)) {
    return res
      .status(400)
      .json({success: false, message: "Minimum of 8 character"});
  }
  if (reply) {
    if (req.body.hash === reply) {
      const salt = await bcrypt.genSalt(12);
      const pass = await bcrypt.hash(req.body.password, salt);
      User.findOneAndUpdate({phone}, {$set: {password: pass}}, {new: true})
        .then(() => {
          res
            .status(200)
    ...
    res
      .status(400)
      .json({success: false, message: "User not found or Timeout"});
  }
};
