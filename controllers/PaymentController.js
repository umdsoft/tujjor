const { BillingErrors, RPCErrors } = require("../utils/payment");
...
exports.payme = async (req, res) => {
    ...
    async function CheckPerformTransaction(id, amount) {
        await Order.findOne({ orderId: id }, (err, data) => {
            if (err || !data)
                return sendResponse(BillingErrors.OrderNotFound(), null);
            if (data.payed === 1)
                ...
                if (!data) {
                    ...
                    await Order.findOne(
                        ...
                        async (err, order) => {
                            if (err || !order)
                                return sendResponse(
                                    BillingErrors.OrderNotFound(),
                                    null
                                );
                            if (order.payed === 1)
                                return sendResponse(
                                    BillingErrors.OrderAvailable(),
                                    null
                                );
        ...
        await Transaction.findOne(
            ...
            async (err, transaction) => {
                ...
                if (transaction.state === 1) {
                    ...
                    //     let param = await Param.findById({
                    //         _id: element.param,
                    //     });
                    //     let temp = [];
                                ...
                                if (order.payed === 0) {
                                    await Transaction.updateOne(
                                        ...
                                        return sendResponse(null, {
                                            state: transac.state,
                                            cancel_time:
                                                transac.cancel_time || 0,
                                            transaction: transac.transaction,
                                            create_time: transac.create_time,
                                            perform_time:
                                                transac.perform_time || 0,
                                        });
                                    });
                                } else {
                                    return sendResponse(
                                        BillingErrors.OrderNotСanceled(),
                                        null
                                    );
                                }
                            }
